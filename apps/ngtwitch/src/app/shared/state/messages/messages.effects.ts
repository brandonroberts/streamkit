...
export class MessagesEffects {
  fetchAvatar$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      filter(([action, avatarUrlsDictionary]) => !avatarUrlsDictionary[action.message.user]),
      concatMap(([action]) => {
        return this.twitchService.getUserProfileUrl(action.message.user)
          .pipe(
            map(profileImageUrl => MessagesActions.messageAvatarFetchedSuccess({
