...
import { YouTubeActions as YouTubeWebSocketActions } from '@streamkit/actions';
...
export class YouTubeEffects {
  startPolling$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      exhaustMap(() => {
        return this.youtubService.getBroadcasts()
          .pipe(
            filter(total => total.length > 0),
            map(broadcasts => {
              return this.router.routerState.snapshot.root.queryParamMap.get('liveChatId') || broadcasts[0].snippet.liveChatId;
            }),
      ...
      switchMap(() =>
        this.actions$.pipe(
          ...
          map(action => {
            ...
            const message = action.message.message.substring(`!${command}`.length);
  ...
  responseToNgRxCommand$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      filter(action => action.command.command === 'ngrx' && this.router.routerState.snapshot.root.queryParamMap.get('respond') === "true"),
      switchMap(action => {
