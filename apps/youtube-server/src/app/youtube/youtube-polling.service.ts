...
export class YouTubePollingService {
  ...
  private async pollMessages(
    ...
    this.youtubeService
      .getLiveChatMessages(liveChatId, nextPageToken)
      .then((response) => {
        const data = response.data;
        const dataMessages = data.items;
        const newMessages = dataMessages.filter(
          (message) => !this.storedMessages.find((msg) => msg.id === message.id)
        );
        ...
        if (newMessages.length) {
          if (first) {
            ...
            this._messages$.next(
              YouTubeWebSocketActions.polledMessages({
                data: { liveChatId, messages: newMessages },
              })
            );
          ...
          this.chatPollIntervalMillis =
            this.chatPollIntervalMillis >= 5000
              ? 5000
              : this.chatPollIntervalMillis + 3000;
        }
      ...
      .catch(() => {
        ...
        if (this.polling) {
          console.log(
            `messages poll failed, next poll in ${this.chatPollIntervalMillis}`
          );
  ...
  private pollSubscriptions(first: boolean) {
    ...
    this.youtubeService
      ...
      .then((response) => {
        ...
        const newSubscriptions = dataSubscriptions.filter(
          (message) =>
            !this.storedSubscriptions.find((msg) => msg.id === message.id)
        );
        this.storedSubscriptions = [
          ...this.storedSubscriptions,
          ...newSubscriptions,
        ];
        ...
        if (newSubscriptions.length) {
          if (first) {
            ...
            this._messages$.next(
              YouTubeWebSocketActions.polledSubscribers({
                data: { subscriptions: newSubscriptions },
              })
            );
          }
  ...
  private checkAndSendCommands(messages: MessageModel[]) {
    messages
      .filter((message) =>
        message.snippet.textMessageDetails.messageText.startsWith('!')
      )
      .map((message) => {
      ...
      .forEach((message: CommandModel) => {
        this._messages$.next(
          YouTubeWebSocketActions.polledCommands({
  ...
  private respondToCommand(message: any) {
    ...
    if (command) {
      ...
      switch (command) {
        ...
        case 'discord': {
          response =
            'Check out our NgRx discord server at https://discord.gg/ngrx';
          break;
        }
      }

      if (response) {
        this.youtubeService.postChatMessage(
          messageInfo.snippet.liveChatId,
          `[StreamKit]: ${response}`
        );
      }
    }
